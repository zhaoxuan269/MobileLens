...
public class SrsPlayer : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        receiveStream.OnAddTrack = e =>
        {
            ...
            if (e.Track is VideoStreamTrack videoTrack)
            {
                videoTrack.OnVideoReceived += tex =>
                {
                    ...
                    var width = tex.width < 1280 ? tex.width : 2388;
                    //var height = tex.width > 0 ? tex.width : 1668;
                    var height = tex.width > 0 ? width * tex.height / tex.width : 1668;
                    receiveImage.rectTransform.sizeDelta = new Vector2(width, height);
                    Color c = receiveImage.color;
                    c.a = 1.0f;
                    receiveImage.color = c;
                };
            }
            if (e.Track is AudioStreamTrack audioTrack)
            {
        ...
        IEnumerator PeerNegotiationNeeded()
        {
            ...
            Debug.Log($"WebRTC: CreateOffer done={op.IsDone}, hasError={op.IsError}, {op.Desc.sdp}");
            if (op.IsError) yield break;
